...
> 版本：v3.2.2 ｜ 文档类别：产品设计 + 技术方案 + 12 周开发计划
...
> 9. （v3.2.2）v0.2 落地对齐：邮箱降级通道细化（验证码接口拆分为「发送 / 校验」两步、新用户首次登录补全资料建档、管理员核验为运营层面事后抽查）；接口总表补充 `/api/auth/email/code`、`/api/auth/logout`、`/api/auth/token` 三个认证接口；`.env` 增加会话/验证码相关配置项。
...
> **邮箱降级通道实现细则（v0.2 落地，v3.2.2 补充）**：
> - 流程两步：`POST /api/auth/email/code` 发送验证码（6 位数字，Redis 存储，10 分钟有效，同学号 60s 重发冷却）→ `POST /api/auth/email/verify` 校验并登录；验证码先校验、登录/建档成功才销毁。
> - 邮箱通道拿不到学籍信息，**新用户首次登录需补全**真实姓名 / 昵称 / 年级 / 专业后建档（真实姓名仅核验用，接口与页面均不下发）；老用户验证码通过即登录。
> - **管理员人工核验**定位为运营层面事后抽查（对照学籍名单核查学号/姓名真实性），不阻塞登录流程，因此不需要额外数据字段。
> - 开发期为假通道：验证码写应用日志且接口响应回显 `dev_code`（`EMAIL_CODE_ECHO=true`，生产必须关闭）；接入真实邮件服务时仅替换发送实现，接口契约不变。
...
| 认证 | `/api/auth/email/code` | POST | 降级：发送校园邮箱验证码（v3.2.2 补充） |
| 认证 | `/api/auth/email/verify` | POST | 降级：校园邮箱验证（新用户补全资料建档） |
| 认证 | `/api/auth/logout` | POST | 退出登录，销毁会话（v3.2.2 补充） |
| 认证 | `/api/auth/token` | POST | 凭会话签发 API 通道 JWT（v3.2.2 补充） |
...
# ---------- 会话与认证（v3.2.2 补充，均有默认值，按需覆盖） ----------
# SESSION_TTL_SECONDS=604800         # Redis Session 有效期（滑动续期）
# JWT_TTL_SECONDS=604800             # API 通道 JWT 有效期
# EMAIL_CODE_TTL_SECONDS=600         # 邮箱验证码有效期
# EMAIL_CODE_COOLDOWN_SECONDS=60     # 验证码重发冷却
# ALLOWED_EMAIL_SUFFIX=              # 校园邮箱域名限制（如 .edu.cn），空为不限制
# EMAIL_CODE_ECHO=true               # 开发期验证码假通道回显，生产必须 false
```
